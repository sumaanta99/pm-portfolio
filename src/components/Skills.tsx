...
export function Skills() {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute left-1/2 top-20 -z-10 hidden h-96 w-96 -translate-x-1/2 rounded-full bg-accent/10 blur-[120px] md:block" />
        ...
        <StaggerGroup className="flex flex-wrap justify-center gap-2.5 sm:gap-3">
          {tools.map((tool) =>
            lightweight ? (
              <span
                key={tool}
                className="cursor-default rounded-full border border-line bg-surface/60 px-4 py-2 text-xs font-medium text-muted sm:px-5 sm:py-2.5 sm:text-sm"
              >
                {tool}
              </span>
            ) : (
