      ...
      <div className="relative aspect-[2/3] w-full overflow-hidden rounded-2xl border border-line bg-elevated shadow-lift">
        {hasCover ? (
          ...
          <div className="flex h-full w-full flex-col justify-end gap-1 bg-gradient-to-br from-surface to-elevated p-4">
            <span className="font-display text-sm font-semibold leading-snug text-ink line-clamp-5">
              {book.title}
            </span>
            {book.author && (
        ...
        <div className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100 [box-shadow:0_0_0_1px_rgb(var(--accent)/0.5),0_30px_60px_-20px_rgb(var(--accent)/0.45)]" />
      </div>

      {/* Meta */}
      <div className="mt-3 px-0.5 sm:mt-3.5">
        <h3 className="line-clamp-2 text-[13px] font-semibold leading-snug tracking-tight text-ink sm:text-sm">
          {book.title}
        </h3>
        {book.author && (
